...
if (-not $makeCmd) {
    ...
    if ($wslCmd) {
        ...
        Write-Host "Native make was not found. Falling back to WSL for the Wishbone pyuvm run."
        & $wslCmd.Source bash -lc "cd '$wslProjectRoot' && bash scripts/run-wishbone-uvm.sh $noWavesArg"
...
$env:COCOTB_TEST_MODULES = "tb.test_simple_cpu_wishbone_pyuvm"
$env:SIM = "icarus"
...
$env:SIM_BUILD = "sim_build/wishbone_pyuvm"
$env:COCOTB_RESULTS_FILE = "sim_build/wishbone_uvm_results.xml"
...
Write-Host "Wishbone pyuvm run complete."
Write-Host "Results XML: sim_build/wishbone_uvm_results.xml"
