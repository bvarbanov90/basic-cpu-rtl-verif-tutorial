...
if ((Test-Path "sim_build/simple_cpu_axi_lite_fault_tb.vcd") -and (-not $NoWaves)) {
    Write-Host "Waveform written to sim_build/simple_cpu_axi_lite_fault_tb.vcd"
}

if (Test-Path "sim_build/axi_lite_fault_coverage.json") {
    Write-Host "AXI-Lite fault coverage JSON: sim_build/axi_lite_fault_coverage.json"
}

if (Test-Path "sim_build/axi_lite_fault_coverage.csv") {
    Write-Host "AXI-Lite fault coverage CSV:  sim_build/axi_lite_fault_coverage.csv"
}

Write-Host "AXI-Lite fault simulation complete."
