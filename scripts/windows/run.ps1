...
if ((Test-Path "sim_build/simple_cpu_tb.vcd") -and (-not $NoWaves)) {
    Write-Host "Waveform written to sim_build/simple_cpu_tb.vcd"
}

if (Test-Path "sim_build/coverage.json") {
    Write-Host "Coverage JSON: sim_build/coverage.json"
}

if (Test-Path "sim_build/coverage.csv") {
    Write-Host "Coverage CSV:  sim_build/coverage.csv"
}
