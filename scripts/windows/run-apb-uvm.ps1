...
if (-not $makeCmd) {
    ...
    if ($wslCmd) {
        ...
        Write-Host "Native make was not found. Falling back to WSL for the APB pyuvm run."
        & $wslCmd.Source bash -lc "cd '$wslProjectRoot' && bash scripts/run-apb-uvm.sh $noWavesArg"
...
$env:COCOTB_TEST_MODULES = "tb.test_simple_cpu_apb_pyuvm"
$env:SIM = "icarus"
...
$env:SIM_BUILD = "sim_build/apb_pyuvm"
$env:COCOTB_RESULTS_FILE = "sim_build/apb_uvm_results.xml"
...
Write-Host "APB pyuvm run complete."
Write-Host "Results XML: sim_build/apb_uvm_results.xml"
