...
if ((Test-Path "sim_build/simple_cpu_apb_fault_tb.vcd") -and (-not $NoWaves)) {
    Write-Host "Waveform written to sim_build/simple_cpu_apb_fault_tb.vcd"
}

if (Test-Path "sim_build/apb_fault_coverage.json") {
    Write-Host "APB fault coverage JSON: sim_build/apb_fault_coverage.json"
}

if (Test-Path "sim_build/apb_fault_coverage.csv") {
    Write-Host "APB fault coverage CSV:  sim_build/apb_fault_coverage.csv"
}

Write-Host "APB fault simulation complete."
