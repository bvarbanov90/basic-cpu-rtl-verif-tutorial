...
if (-not $makeCmd) {
    ...
    if ($wslCmd) {
        ...
        Write-Host "Native make was not found. Falling back to WSL for the MMIO wait-state pyuvm run."
        & $wslCmd.Source bash -lc "cd '$wslProjectRoot' && bash scripts/run-mmio-wait-uvm.sh $noWavesArg"
...
$env:SIM_BUILD = "sim_build/mmio_wait_pyuvm"
$env:COCOTB_RESULTS_FILE = "sim_build/mmio_wait_uvm_results.xml"
...
Write-Host "MMIO wait-state pyuvm run complete."
Write-Host "Results XML: sim_build/mmio_wait_uvm_results.xml"
