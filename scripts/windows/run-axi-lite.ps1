...
if ((Test-Path "sim_build/simple_cpu_axi_lite_tb.vcd") -and (-not $NoWaves)) {
    Write-Host "Waveform written to sim_build/simple_cpu_axi_lite_tb.vcd"
}

if (Test-Path "sim_build/axi_lite_coverage.json") {
    Write-Host "AXI-Lite coverage JSON: sim_build/axi_lite_coverage.json"
}

if (Test-Path "sim_build/axi_lite_coverage.csv") {
    Write-Host "AXI-Lite coverage CSV:  sim_build/axi_lite_coverage.csv"
}

Write-Host "AXI-Lite simulation complete."
