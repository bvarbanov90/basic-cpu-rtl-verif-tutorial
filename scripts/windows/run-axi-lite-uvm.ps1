...
if (-not $makeCmd) {
    ...
    if ($wslCmd) {
        ...
        Write-Host "Native make was not found. Falling back to WSL for the AXI-Lite pyuvm run."
        & $wslCmd.Source bash -lc "cd '$wslProjectRoot' && bash scripts/run-axi-lite-uvm.sh $noWavesArg"
...
$env:COCOTB_TEST_MODULES = "tb.test_simple_cpu_axi_lite_pyuvm"
$env:SIM = "icarus"
...
$env:SIM_BUILD = "sim_build/axi_lite_pyuvm"
$env:COCOTB_RESULTS_FILE = "sim_build/axi_lite_uvm_results.xml"
...
Write-Host "AXI-Lite pyuvm run complete."
Write-Host "Results XML: sim_build/axi_lite_uvm_results.xml"
