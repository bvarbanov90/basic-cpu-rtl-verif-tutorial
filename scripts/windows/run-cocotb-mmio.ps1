...
if (-not $makeCmd) {
    ...
    if ($wslCmd) {
        ...
        Write-Host "Native make was not found. Falling back to WSL for the cocotb MMIO run."
        & $wslCmd.Source bash -lc "cd '$wslProjectRoot' && bash scripts/run-cocotb-mmio.sh $noWavesArg"
    ...
    throw "make is required for cocotb MMIO runs. Install GNU Make or use WSL."
}
...
if (-not $pythonBin) {
    throw "Python is required for cocotb MMIO runs. Install Python 3.13 and retry."
}
...
$env:SIM_BUILD = "sim_build/mmio_cocotb"
$env:COCOTB_RESULTS_FILE = "sim_build/mmio_cocotb_results.xml"
...
Write-Host "cocotb MMIO run complete."
Write-Host "Results XML: sim_build/mmio_cocotb_results.xml"
