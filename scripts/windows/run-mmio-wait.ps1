...
if ((Test-Path "sim_build/simple_cpu_mmio_wait_tb.vcd") -and (-not $NoWaves)) {
    Write-Host "Waveform written to sim_build/simple_cpu_mmio_wait_tb.vcd"
}

if (Test-Path "sim_build/mmio_wait_coverage.json") {
    Write-Host "MMIO-wait coverage JSON: sim_build/mmio_wait_coverage.json"
}

if (Test-Path "sim_build/mmio_wait_coverage.csv") {
    Write-Host "MMIO-wait coverage CSV:  sim_build/mmio_wait_coverage.csv"
}

Write-Host "MMIO-wait simulation complete."
