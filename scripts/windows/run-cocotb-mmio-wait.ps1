...
if (-not $makeCmd) {
    ...
    if ($wslCmd) {
        ...
        Write-Host "Native make was not found. Falling back to WSL for the cocotb MMIO wait-state run."
        & $wslCmd.Source bash -lc "cd '$wslProjectRoot' && bash scripts/run-cocotb-mmio-wait.sh $noWavesArg"
    ...
    throw "make is required for cocotb MMIO wait-state runs. Install GNU Make or use WSL."
}
...
if (-not $pythonBin) {
    throw "Python is required for cocotb MMIO wait-state runs. Install Python 3.13 and retry."
}
...
$env:COCOTB_TEST_MODULES = "tb.test_simple_cpu_mmio"
$env:SIM = "icarus"
$env:TOPLEVEL = "simple_cpu_mmio_wait"
$env:VERILOG_SOURCES = "$projectRootUnix/rtl/simple_cpu.sv $projectRootUnix/rtl/simple_cpu_mmio.sv $projectRootUnix/rtl/simple_cpu_mmio_wait.sv"
$env:SIM_BUILD = "sim_build/mmio_wait_cocotb"
$env:COCOTB_RESULTS_FILE = "sim_build/mmio_wait_cocotb_results.xml"
...
Write-Host "cocotb MMIO wait-state run complete."
Write-Host "Results XML: sim_build/mmio_wait_cocotb_results.xml"
