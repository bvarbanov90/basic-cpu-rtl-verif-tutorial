...
$adapterContractArgs = @()
$adapterContractArgs += $pythonCmd.Arguments
$adapterContractArgs += @("scripts/adapter_contract.py", "--check")
& $pythonCmd.Executable @adapterContractArgs
if ($LASTEXITCODE -ne 0) {
    exit $LASTEXITCODE
}

$referenceRegressionArgs = @()
