...
if (-not $makeCmd) {
    ...
    if ($wslCmd) {
        ...
        Write-Host "Native make was not found. Falling back to WSL for the cocotb AXI-Lite run."
        & $wslCmd.Source bash -lc "cd '$wslProjectRoot' && bash scripts/run-cocotb-axi-lite.sh $noWavesArg"
    ...
    throw "make is required for cocotb AXI-Lite runs. Install GNU Make or use WSL."
}
...
if (-not $pythonBin) {
    throw "Python is required for cocotb AXI-Lite runs. Install Python 3.13 and retry."
}
...
$env:COCOTB_TEST_MODULES = "tb.test_simple_cpu_axi_lite"
$env:SIM = "icarus"
$env:TOPLEVEL = "simple_cpu_axi_lite"
$env:VERILOG_SOURCES = "$projectRootUnix/rtl/simple_cpu.sv $projectRootUnix/rtl/simple_cpu_mmio.sv $projectRootUnix/rtl/simple_cpu_axi_lite.sv"
$env:SIM_BUILD = "sim_build/axi_lite_cocotb"
$env:COCOTB_RESULTS_FILE = "sim_build/axi_lite_cocotb_results.xml"
...
Write-Host "cocotb AXI-Lite run complete."
Write-Host "Results XML: sim_build/axi_lite_cocotb_results.xml"
