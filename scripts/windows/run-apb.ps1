...
if ((Test-Path "sim_build/simple_cpu_apb_tb.vcd") -and (-not $NoWaves)) {
    Write-Host "Waveform written to sim_build/simple_cpu_apb_tb.vcd"
}

if (Test-Path "sim_build/apb_coverage.json") {
    Write-Host "APB coverage JSON: sim_build/apb_coverage.json"
}

if (Test-Path "sim_build/apb_coverage.csv") {
    Write-Host "APB coverage CSV:  sim_build/apb_coverage.csv"
}

Write-Host "APB simulation complete."
