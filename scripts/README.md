...
29. `check-generated-docs` (`check-generated-docs.ps1` / `check-generated-docs.sh`)
30. `check-native` (`check-native.ps1` / `check-native.sh`)
31. `check-all` (`check-all.ps1` / `check-all.sh`)
32. `check-coverage-delta` (`check-coverage-delta.ps1` / `check-coverage-delta.sh`)
33. `update-coverage-baseline` (`update-coverage-baseline.ps1` / `update-coverage-baseline.sh`)
34. `run-formal` (`run-formal.ps1` / `run-formal.sh`)
35. `run-cocotb-verilator` (`run-cocotb-verilator.ps1` / `run-cocotb-verilator.sh`)
36. `run-cocotb-mmio` (`run-cocotb-mmio.ps1` / `run-cocotb-mmio.sh`)
37. `run-mmio-uvm` (`run-mmio-uvm.ps1` / `run-mmio-uvm.sh`)
38. `run-cocotb-mmio-wait` (`run-cocotb-mmio-wait.ps1` / `run-cocotb-mmio-wait.sh`)
39. `run-mmio-wait-uvm` (`run-mmio-wait-uvm.ps1` / `run-mmio-wait-uvm.sh`)
40. `run-cocotb-apb` (`run-cocotb-apb.ps1` / `run-cocotb-apb.sh`)
41. `run-apb-uvm` (`run-apb-uvm.ps1` / `run-apb-uvm.sh`)
42. `run-cocotb-wishbone` (`run-cocotb-wishbone.ps1` / `run-cocotb-wishbone.sh`)
43. `run-wishbone-uvm` (`run-wishbone-uvm.ps1` / `run-wishbone-uvm.sh`)
44. `run-cocotb-axi-lite` (`run-cocotb-axi-lite.ps1` / `run-cocotb-axi-lite.sh`)
45. `run-axi-lite-uvm` (`run-axi-lite-uvm.ps1` / `run-axi-lite-uvm.sh`)
46. `run-equiv` (`run-equiv.ps1` / `run-equiv.sh`)
47. `update-equivalence-golden` (`update-equivalence-golden.ps1` / `update-equivalence-golden.sh`)
48. `format-sv` (`format-sv.ps1` / `format-sv.sh`)
49. `lint` (`lint.ps1` / `lint.sh`)
50. `show-static-analysis` (`show-static-analysis.ps1` / `show-static-analysis.sh`)
51. `open-waves` (`open-waves.ps1` / `open-waves.sh`)
52. `show-model-trace` (`show-model-trace.ps1` / `show-model-trace.sh`)
53. `show-coverage` (`show-coverage.ps1` / `show-coverage.sh`)
54. `show-pyuvm-coverage` (`show-pyuvm-coverage.ps1` / `show-pyuvm-coverage.sh`)
55. `show-mmio-coverage` (`show-mmio-coverage.ps1` / `show-mmio-coverage.sh`)
56. `show-mmio-wait-coverage` (`show-mmio-wait-coverage.ps1` / `show-mmio-wait-coverage.sh`)
57. `show-apb-coverage` (`show-apb-coverage.ps1` / `show-apb-coverage.sh`)
58. `show-wishbone-coverage` (`show-wishbone-coverage.ps1` / `show-wishbone-coverage.sh`)
59. `show-axi-lite-coverage` (`show-axi-lite-coverage.ps1` / `show-axi-lite-coverage.sh`)
60. `show-axi-lite-fault-coverage` (`show-axi-lite-fault-coverage.ps1` / `show-axi-lite-fault-coverage.sh`)
61. `show-apb-fault-coverage` (`show-apb-fault-coverage.ps1` / `show-apb-fault-coverage.sh`)
62. `show-wishbone-fault-coverage` (`show-wishbone-fault-coverage.ps1` / `show-wishbone-fault-coverage.sh`)
63. `show-verilator-coverage` (`show-verilator-coverage.ps1` / `show-verilator-coverage.sh`)
64. `record-coverage-history` (`record-coverage-history.ps1` / `record-coverage-history.sh`)
65. `show-coverage-trend` (`show-coverage-trend.ps1` / `show-coverage-trend.sh`)
66. `show-mutations` (`show-mutations.ps1` / `show-mutations.sh`)
67. `show-formal-status` (`show-formal-status.ps1` / `show-formal-status.sh`)
68. `export-status` (`export-status.ps1` / `export-status.sh`)
