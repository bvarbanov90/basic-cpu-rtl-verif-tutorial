...
def coverage_summary(path: Path, kind: str) -> dict[str, Any]:
    ...
    if kind == "core":
        ...
        payload.update(
            {
                ...
                "setup_phases": int(report.get("setup_phases", 0)),
                "access_phases": int(report.get("access_phases", 0)),
                "partial_write_attempts": int(report.get("partial_write_attempts", 0)),
                "coverage_pass": int(report.get("coverage_pass", 0)),
            }
        )
    elif kind == "apb_fault":
        ...
        payload.update(
            {
                "setup_only_writes_ignored": int(report.get("setup_only_writes_ignored", 0)),
                "aborted_writes_ignored": int(report.get("aborted_writes_ignored", 0)),
                "setup_only_starts_ignored": int(report.get("setup_only_starts_ignored", 0)),
                "penable_without_select_ignored": int(report.get("penable_without_select_ignored", 0)),
    ...
    elif kind == "wishbone_fault":
        payload["required"] = False
        payload.update(
            {
                "cycle_only_writes_ignored": int(report.get("cycle_only_writes_ignored", 0)),
                "aborted_writes_ignored": int(report.get("aborted_writes_ignored", 0)),
                "cycle_only_starts_ignored": int(report.get("cycle_only_starts_ignored", 0)),
                "strobe_without_cycle_ignored": int(report.get("strobe_without_cycle_ignored", 0)),
                "shadow_fault_write_observed": int(report.get("shadow_fault_write_observed", 0)),
                "deferred_shadow_updates": int(report.get("deferred_shadow_updates", 0)),
                "reload_observed_updates": int(report.get("reload_observed_updates", 0)),
                "program_runs": int(report.get("program_runs", 0)),
        ...
        payload.update(
            {
                ...
                "shadow_reads": int(report.get("shadow_reads", 0)),
                "status_reads": int(report.get("status_reads", 0)),
                "read_transactions": int(report.get("read_transactions", 0)),
                "write_transactions": int(report.get("write_transactions", 0)),
