...
COVERAGE_FILE="${1:-sim_build/apb_coverage.json}"

if [[ ! -f "${COVERAGE_FILE}" ]]; then
    echo "APB coverage file not found: ${COVERAGE_FILE}. Run bash scripts/run-apb.sh first." >&2
    exit 1
fi
