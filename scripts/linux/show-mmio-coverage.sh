...
COVERAGE_FILE="${1:-sim_build/mmio_coverage.json}"

if [[ ! -f "${COVERAGE_FILE}" ]]; then
    echo "MMIO coverage file not found: ${COVERAGE_FILE}. Run bash scripts/run-mmio.sh first." >&2
    exit 1
fi
