...
usage() {
    cat <<'EOF'
Usage: bash scripts/run-cocotb-mmio-wait.sh [--no-waves]
EOF
}
...
if ! command -v make >/dev/null 2>&1; then
    echo "make is required for cocotb MMIO wait-state runs." >&2
    exit 1
fi
...
if [[ -x ".venv/bin/python" ]]; then
    ...
    echo "python3 is required for cocotb MMIO wait-state runs." >&2
    exit 1
fi
...
make -f Makefile \
    ...
    WAVES="${WAVES_VALUE}" \
    SIM_BUILD=sim_build/mmio_wait_cocotb \
    COCOTB_RESULTS_FILE=sim_build/mmio_wait_cocotb_results.xml

echo "cocotb MMIO wait-state run complete."
echo "Results XML: sim_build/mmio_wait_cocotb_results.xml"
