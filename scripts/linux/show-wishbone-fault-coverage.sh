...
COVERAGE_FILE="${1:-sim_build/wishbone_fault_coverage.json}"

if [[ ! -f "${COVERAGE_FILE}" ]]; then
    echo "Wishbone fault coverage file not found: ${COVERAGE_FILE}. Run bash scripts/run-wishbone-fault.sh first." >&2
    exit 1
fi
...
print("Wishbone Fault Coverage Summary")
print(f"  pass:                          {cov.get('coverage_pass')}")
print(f"  cycle_only_writes_ignored:     {cov.get('cycle_only_writes_ignored')}")
print(f"  aborted_writes_ignored:        {cov.get('aborted_writes_ignored')}")
print(f"  cycle_only_starts_ignored:     {cov.get('cycle_only_starts_ignored')}")
print(f"  strobe_without_cycle_ignored:  {cov.get('strobe_without_cycle_ignored')}")
