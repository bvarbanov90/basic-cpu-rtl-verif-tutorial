...
usage() {
    cat <<'EOF'
Usage: bash scripts/run-mmio-uvm.sh [--no-waves]
EOF
}
...
make -f Makefile \
    ...
    WAVES="${WAVES_VALUE}" \
    SIM_BUILD=sim_build/mmio_pyuvm \
    COCOTB_RESULTS_FILE=sim_build/mmio_uvm_results.xml

echo "MMIO pyuvm run complete."
echo "Results XML: sim_build/mmio_uvm_results.xml"
