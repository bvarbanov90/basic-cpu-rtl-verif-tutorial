...
COVERAGE_FILE="${1:-sim_build/axi_lite_coverage.json}"

if [[ ! -f "${COVERAGE_FILE}" ]]; then
    echo "AXI-Lite coverage file not found: ${COVERAGE_FILE}. Run bash scripts/run-axi-lite.sh first." >&2
    exit 1
fi
...
print("AXI-Lite Coverage Summary")
...
print(f"  partial_write_attempts:{cov.get('partial_write_attempts')}")
