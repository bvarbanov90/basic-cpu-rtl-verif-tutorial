...
| `show-pyuvm-coverage` | `scripts/show-pyuvm-coverage.*` | Summarizes `sim_build/pyuvm_coverage.json` through the same core coverage report view as the native SV core report. |
...
| `test_coverage_goals_unit` | `tb/test_coverage_goals_unit.py` | Pytest checks that native SV coverage-goal report blocks, Python coverage-model goals, artifacts, and show helpers stay aligned. |
| `generate-coverage-goals` | `scripts/coverage_goals.py` + wrappers | Generates and checks `docs/coverage-goals.md` from executable functional coverage goal definitions. |
| `test_reference_regression_unit` | `tb/test_reference_regression_unit.py` | Pytest checks that deterministic directed and seeded random Python programs halt and close the reference coverage model. |
| `generate-reference-regression` | `scripts/reference_regression_catalog.py` + wrappers | Generates and checks `docs/reference-regression.md` from deterministic reference-model regression cases. |
| `test_semantic_vectors_unit` | `tb/test_semantic_vectors_unit.py` | Pytest checks single-instruction pre-state/post-state semantic vectors for every opcode plus illegal-opcode and flag edge cases. |
| `generate-semantic-vectors` | `scripts/semantic_vectors.py` + wrappers | Generates and checks `docs/semantic-vectors.md` from executable reference-model semantic vectors. |
| `test_register_map_unit` | `tb/test_register_map_unit.py` | Pytest checks that wrapper address constants match between RTL and Python bus-functional models. |
| `generate-register-map` | `scripts/register_map.py` + wrappers | Generates and checks `docs/register-map.md` from the shared wrapper address contract. |
| `test_protocol_catalog_unit` | `tb/test_protocol_catalog_unit.py` | Pytest checks that shared protocol conformance scenarios halt under the reference model and are called by every adapter test. |
| `generate-protocol-catalog` | `scripts/protocol_catalog.py` + wrappers | Generates and checks `docs/protocol-conformance.md` from the shared protocol conformance scenario library. |
| `test_adapter_contract_unit` | `tb/test_adapter_contract_unit.py` | Pytest checks that direct-core, MMIO, APB, Wishbone, and AXI-Lite Python bus adapters expose the expected conformance method surface. |
| `generate-adapter-contract` | `scripts/adapter_contract.py` + wrappers | Generates and checks `docs/adapter-contract.md` from the static Python bus-adapter API contract. |
| `test_documentation_index_unit` | `tb/test_documentation_index_unit.py` | Pytest checks that tracked Markdown docs, generated catalogs, exported reports, data docs, and generator wrappers stay indexed. |
| `generate-documentation-index` | `scripts/documentation_index.py` + wrappers | Generates and checks `docs/documentation-index.md` from the tutorial documentation map. |
...
10. `docs/coverage-goals.md` (generated functional coverage-goal catalog)
11. `docs/reference-regression.md` (generated deterministic Python reference-regression catalog)
12. `docs/semantic-vectors.md` (generated single-instruction semantic vector catalog)
13. `docs/register-map.md` (generated wrapper register-map catalog)
14. `docs/protocol-conformance.md` (generated shared protocol-conformance scenario catalog)
15. `docs/adapter-contract.md` (generated Python bus-adapter API contract)
16. `docs/documentation-index.md` (generated documentation and generated-artifact review map)
...
19. `scripts/show-pyuvm-coverage.ps1` / `scripts/show-pyuvm-coverage.sh` summarize the pyuvm subscriber coverage report without opening JSON manually.
20. CI workflow is in `.github/workflows/ci.yml` and is split into python-model, native-sim, lint, formal, cocotb-verilator, equivalence, pyuvm, mutations, and summary jobs; the `python-model` job checks the executable reference spec before longer simulator jobs, the `native-sim` job runs the direct core, MMIO, MMIO wait-state, APB, Wishbone, AXI-Lite, APB fault, Wishbone fault, and AXI-Lite fault native lanes plus corpus replay through APB, Wishbone, AXI-Lite, and MMIO wait-state, while the `pyuvm` job runs the direct pyuvm, MMIO pyuvm, MMIO cocotb, MMIO wait-state cocotb, MMIO wait-state pyuvm, APB cocotb, APB pyuvm, Wishbone cocotb, Wishbone pyuvm, AXI-Lite cocotb, and AXI-Lite pyuvm regressions.
21. Baseline comparisons are run through `scripts/check-coverage-delta.ps1` and `scripts/check-coverage-delta.sh`.
22. `scripts/show-formal-status.ps1` / `scripts/show-formal-status.sh` summarize formal target health and solver/runtime metadata across core, MMIO, MMIO wait-state, MMIO wait-state fault, APB, APB-fault, Wishbone, Wishbone-fault, AXI-Lite, AXI-Lite-fault, and cover targets.
23. `scripts/export-status.ps1` / `scripts/export-status.sh` export repo-tracked verification status Markdown/JSON plus badge endpoint payloads, including optional suite summaries such as `mmio_wait_coverage`, `pyuvm_coverage`, `cocotb_verilator`, `mmio_cocotb`, `mmio_pyuvm`, `mmio_wait_cocotb`, `mmio_wait_pyuvm`, `apb_coverage`, `apb_fault_coverage`, `apb_cocotb`, `apb_pyuvm`, `wishbone_coverage`, `wishbone_fault_coverage`, `wishbone_cocotb`, `wishbone_pyuvm`, `axi_lite_coverage`, `axi_lite_fault_coverage`, `axi_lite_cocotb`, and `axi_lite_pyuvm`.
24. `tb/protocol_conformance.py` is the shared scenario library that replays the same smoke, logic, loop, branch-stress, and randomized programs across the direct core, MMIO, APB, Wishbone, and AXI-Lite buses.
25. `tb/core_bus.py`, `tb/mmio_bus.py`, `tb/apb_bus.py`, `tb/wishbone_bus.py`, and `tb/axi_lite_bus.py` expose the same high-level load/start/sample interface so the shared conformance suite can verify wrapper parity instead of only per-wrapper local tests.
26. `formal/simple_cpu_mmio.sby` swaps in an abstract `simple_cpu` stub so the wrapper proof focuses on MMIO control/address behavior instead of re-proving CPU internals.
27. `formal/simple_cpu_mmio_wait.sby` swaps in an abstract `simple_cpu_mmio` stub so the wait-state proof focuses on request latching, fixed-cycle delay, and delayed read-data pass-through instead of re-proving MMIO internals.
28. `formal/simple_cpu_mmio_wait_faults.sby` keeps a separate wait-state fault proof focused on captured-request integrity under later external bus glitches and on forbidding early `bus_ready` responses.
29. `formal/simple_cpu_apb.sby` swaps in an abstract `simple_cpu_mmio` stub so the APB proof focuses on setup/access handshake behavior and MMIO read-data pass-through instead of re-proving MMIO internals.
30. `formal/simple_cpu_apb_faults.sby` keeps APB fault proofs separate from the baseline APB shell contract by driving deterministic setup/glitch/reload sequences through a small stateful MMIO stub.
31. The dedicated `formal/*cover_formal.sv` harnesses keep `sby cover` focused on one witness goal per target so trace generation stays fast.
32. `scripts/run-cocotb-verilator.sh` auto-uses a cached Linux OSS CAD Suite Verilator when the distro package is too old for cocotb.
33. `scripts/linux/oss-cad-suite.sh` centralizes Linux OSS CAD Suite release selection, retrying GitHub release queries/downloads and honoring `OSS_CAD_SUITE_RELEASE_TAG` for reproducible CI runs.
34. `scripts/static_analysis.py` is the single entry point behind `lint`, `show-static-analysis`, and `format-sv`.
35. `equiv/simple_cpu_golden.sv` is the tracked golden snapshot; refresh it intentionally with `scripts/update-equivalence-golden.*` only when the new RTL behavior is meant to become the baseline.
36. `formal/simple_cpu_wishbone.sby` swaps in an abstract `simple_cpu_mmio` stub so the Wishbone proof focuses on `CYC/STB/ACK` translation and MMIO read-data pass-through instead of re-proving MMIO internals.
37. `formal/simple_cpu_wishbone_faults.sby` keeps Wishbone fault proofs separate from the baseline Wishbone shell contract by driving deterministic `CYC`-only, `STB`-without-`CYC`, and reload sequences through a small stateful MMIO stub.
38. `formal/simple_cpu_axi_lite.sby` swaps in an abstract `simple_cpu_mmio` stub so the AXI-Lite proof focuses on coupled-channel write acceptance, response hold/clear, partial-write rejection, and registered read-data capture.
39. `formal/simple_cpu_axi_lite_faults.sby` keeps AXI-Lite fault proofs separate from the baseline AXI-Lite shell contract by driving deterministic `AWVALID`-only, `WVALID`-only, split-channel, pending-response, and reload sequences through a small stateful MMIO stub.
40. Verible intentionally excludes `rtl/simple_cpu_mmio.sv`, `rtl/simple_cpu_mmio_wait.sv`, `rtl/simple_cpu_apb.sv`, `rtl/simple_cpu_wishbone.sv`, and `rtl/simple_cpu_axi_lite.sv`; those files remain covered by Verilator lint and svlint.
41. `tb/cpu_lib.py` exposes disassembly and cycle-by-cycle trace helpers so the Python reference model can be used as both a scoreboard oracle and a human-readable debug tool.
42. `docs/isa.md` is generated by `scripts/isa_report.py` and checked in CI so opcode metadata remains aligned across the assembler and Python reference model.
43. `docs/assembler-regressions.md` is generated by `scripts/asm_corpus_report.py` and checked in CI so the tracked assembly corpus has a readable, non-stale summary.
44. `docs/formal-targets.md` is generated by `scripts/formal_catalog.py` and checked in CI so `.sby` target metadata and run-formal wrapper coverage remain aligned.
45. `docs/script-catalog.md` is generated by `scripts/script_catalog.py` and checked in CI so top-level wrappers, platform implementations, and `scripts/README.md` common entrypoints remain aligned.
46. `docs/ci-catalog.md` is generated by `scripts/ci_catalog.py` and checked in CI so GitHub Actions job names, summary fan-in, script references, artifact metadata, and the pinned OSS CAD Suite release remain reviewable and valid.
47. `docs/tooling-catalog.md` is generated by `scripts/tooling_catalog.py` and checked in CI so Python requirements, installer profiles, GitHub release sources, environment variables, and OSS CAD Suite pins remain reviewable and valid.
48. `docs/verification-matrix.md` is generated by `scripts/verification_matrix.py` and checked in CI so native SV, cocotb, pyuvm, formal, assertion, artifact, and mutation-bench coverage remain aligned across lanes.
49. `docs/artifact-catalog.md` is generated by `scripts/artifact_catalog.py` and checked in CI so result files, coverage reports, formal workdirs, CI upload paths, and show-helper wrappers remain aligned.
50. `docs/requirements-traceability.md` is generated by `scripts/requirements_traceability.py` and checked in CI so tutorial verification claims remain connected to executable lanes, formal targets, mutation benches, and artifacts.
51. `docs/coverage-goals.md` is generated by `scripts/coverage_goals.py` and checked in CI so explicit coverage goals, thresholds, source testbenches, artifacts, and show-helper commands remain aligned.
52. `docs/reference-regression.md` is generated by `scripts/reference_regression_catalog.py` and checked in CI so deterministic directed and seeded-random Python reference programs keep their expected final states and combined coverage closure reviewable.
53. `docs/semantic-vectors.md` is generated by `scripts/semantic_vectors.py` and checked in CI so opcode-level pre-state/post-state behavior, flags, memory effects, branches, and illegal-opcode handling remain reviewable.
54. `docs/register-map.md` is generated by `scripts/register_map.py` and checked in CI so wrapper address semantics remain aligned between RTL and Python bus-functional models.
55. `docs/protocol-conformance.md` is generated by `scripts/protocol_catalog.py` and checked in CI so shared conformance scenarios, expected final states, and adapter test coverage stay reviewable.
56. `docs/adapter-contract.md` is generated by `scripts/adapter_contract.py` and checked in CI so direct-core and wrapper Python bus-functional adapters keep the method surface expected by shared conformance, directed cocotb, and pyuvm tests.
57. `docs/documentation-index.md` is generated by `scripts/documentation_index.py` and checked in CI so manual docs, generated catalogs, exported reports, and data docs stay discoverable with their source-of-truth files.
58. `docs/mutation-catalog.md` is generated by `scripts/mutation_catalog.py` and checked in CI so mutation definitions, bench scopes, and RTL replacement snippets remain reviewable and valid.
59. `tb/mmio_bus.py` factors the cocotb MMIO reset/read/write/control helpers into a reusable Python bus-functional layer and now waits for delayed `bus_ready`, so the same helper can drive both the always-ready and wait-state wrappers.
60. `tb/apb_bus.py` is the parallel reusable Python bus-functional layer for the APB shell, keeping the higher-level control/status semantics aligned with MMIO while checking a real setup/access handshake.
61. `tb/wishbone_bus.py` is the parallel reusable Python bus-functional layer for the Wishbone shell, keeping the higher-level control/status semantics aligned with MMIO while checking a real `CYC/STB/ACK` handshake.
