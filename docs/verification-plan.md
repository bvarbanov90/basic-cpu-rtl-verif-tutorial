...
11. `docs/register-map.md` (generated wrapper register-map catalog)
12. `docs/protocol-conformance.md` (generated shared protocol-conformance scenario catalog)
13. `docs/adapter-contract.md` (generated Python bus-adapter API contract)
14. `docs/documentation-index.md` (generated documentation and generated-artifact review map)
...
52. `docs/register-map.md` is generated by `scripts/register_map.py` and checked in CI so wrapper address semantics remain aligned between RTL and Python bus-functional models.
53. `docs/protocol-conformance.md` is generated by `scripts/protocol_catalog.py` and checked in CI so shared conformance scenarios, expected final states, and adapter test coverage stay reviewable.
54. `docs/adapter-contract.md` is generated by `scripts/adapter_contract.py` and checked in CI so direct-core and wrapper Python bus-functional adapters keep the method surface expected by shared conformance, directed cocotb, and pyuvm tests.
55. `docs/documentation-index.md` is generated by `scripts/documentation_index.py` and checked in CI so manual docs, generated catalogs, exported reports, and data docs stay discoverable with their source-of-truth files.
56. `docs/mutation-catalog.md` is generated by `scripts/mutation_catalog.py` and checked in CI so mutation definitions, bench scopes, and RTL replacement snippets remain reviewable and valid.
57. `tb/mmio_bus.py` factors the cocotb MMIO reset/read/write/control helpers into a reusable Python bus-functional layer and now waits for delayed `bus_ready`, so the same helper can drive both the always-ready and wait-state wrappers.
58. `tb/apb_bus.py` is the parallel reusable Python bus-functional layer for the APB shell, keeping the higher-level control/status semantics aligned with MMIO while checking a real setup/access handshake.
59. `tb/wishbone_bus.py` is the parallel reusable Python bus-functional layer for the Wishbone shell, keeping the higher-level control/status semantics aligned with MMIO while checking a real `CYC/STB/ACK` handshake.
