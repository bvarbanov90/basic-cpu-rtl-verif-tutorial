...
class WishboneProgramItem(uvm_sequence_item):
    def __init__(
        self,
        name="wishbone_program_item",
        program=None,
...
class WishboneLogicOpsSequence(uvm_sequence):
    async def body(self):
        item = WishboneProgramItem(
            "wishbone_logic_ops_item",
...
class WishboneControlStatusSequence(uvm_sequence):
    async def body(self):
        program = [
            ins(OPC_LDI, 1),
            ins(OPC_STA, 0),
...
class WishboneRandomProgramSequence(uvm_sequence):
    def __init__(self, name="wishbone_random_program_sequence", seed=20260411):
        super().__init__(name)
        self.seed = int(seed)

    async def body(self):
        item = WishboneProgramItem(
            "wishbone_random_item",
...
class SimpleCpuWishboneBfm(uvm_object):
    def __init__(self, name="simple_cpu_wishbone_bfm", dut=None):
        super().__init__(name)
        self.dut = dut
        self._clock_started = False
        self.wishbone = SimpleCpuWishboneBus(dut)

    async def ensure_clock(self):
        if self._clock_started:
            return
        cocotb.start_soon(Clock(self.dut.wb_clk_i, 10, unit="ns").start())
        self._clock_started = True
        await Timer(1, unit="ns")

    async def reset(self):
        await self.ensure_clock()
        await self.wishbone.reset()
    ...
    async def start_with_optional_state_observation(self, observe_control_states: bool, max_cycles: int) -> tuple[int, int]:
        if not observe_control_states:
            await self.wishbone.start_program()
            return 0, 1

        await self.wishbone.write(0x30, 0x01)
        seen_load = 0
        seen_run = 0
        for _ in range(max_cycles):
            control = await self.wishbone.read(0x30)
            if (control & 0x2) != 0:
                seen_load = 1
            if (control & 0x1) != 0:
                seen_run = 1
    ...
    async def run_item(self, item: WishboneProgramItem) -> WishboneRunTrace:
        await self.reset()
        await self.wishbone.load_shadow_program(item.program)

        if item.verify_shadow_readback:
            for addr, value in enumerate(item.program[:16]):
                observed = await self.wishbone.read(addr)
        ...
        await self.wishbone.run_until_halt(max_cycles=item.max_cycles)
        snapshot = await self.wishbone.sample_state()
        return WishboneRunTrace(
...
class SimpleCpuWishboneDriver(uvm_driver):
    def build_phase(self):
        self.bfm = ConfigDB().get(self, "", "WISHBONE_CPU_BFM")
...
class SimpleCpuWishboneScoreboard(uvm_component):
    def build_phase(self):
    ...
    @staticmethod
    def assert_equal(trace: WishboneRunTrace, field: str, observed, expected):
    ...
    async def run_phase(self):
        while self.checked_runs < self.expected_runs:
            ...
            if trace.item_name == "wishbone_control_status_item":
...
class SimpleCpuWishboneAgent(uvm_component):
    def build_phase(self):
        self.sequencer = uvm_sequencer("sequencer", self)
        self.driver = SimpleCpuWishboneDriver("driver", self)

    def connect_phase(self):
        self.driver.seq_item_port.connect(self.sequencer.seq_item_export)


class SimpleCpuWishboneEnv(uvm_component):
    def build_phase(self):
        self.expected_runs = int(get_config(self, "EXPECTED_RUNS", 1))
        self.agent = SimpleCpuWishboneAgent("agent", self)
        self.scoreboard = SimpleCpuWishboneScoreboard("scoreboard", self)
    ...
    async def wait_for_completion(self, timeout_cycles=1024):
        for _ in range(timeout_cycles):
            if self.scoreboard.checked_runs >= self.expected_runs:
                return
            await RisingEdge(cocotb.top.wb_clk_i)
        raise AssertionError("Wishbone pyuvm environment did not complete in time")


class SimpleCpuWishboneBaseTest(uvm_test):
    expected_runs = 1

    def build_phase(self):
        bfm = SimpleCpuWishboneBfm(dut=cocotb.top)
        ConfigDB().set(None, "*", "WISHBONE_CPU_BFM", bfm)
        ConfigDB().set(None, "*", "EXPECTED_RUNS", self.expected_runs)
        self.bfm = bfm
        self.env = SimpleCpuWishboneEnv("env", self)
...
@test()
class SimpleCpuWishboneUvmSmokeTest(SimpleCpuWishboneBaseTest):
    async def run_phase(self):
        self.raise_objection()
        await WishboneLogicOpsSequence("wishbone_logic_ops_seq").start(self.env.agent.sequencer)
        await self.env.wait_for_completion()
        self.drop_objection()


@test()
class SimpleCpuWishboneUvmRandomizedTest(SimpleCpuWishboneBaseTest):
    async def run_phase(self):
        self.raise_objection()
        await WishboneRandomProgramSequence("wishbone_random_seq", seed=20260411).start(self.env.agent.sequencer)
        await self.env.wait_for_completion(timeout_cycles=4096)
        self.drop_objection()


@test()
class SimpleCpuWishboneUvmControlStatusTest(SimpleCpuWishboneBaseTest):
    async def run_phase(self):
        self.raise_objection()
        await WishboneControlStatusSequence("wishbone_control_status_seq").start(self.env.agent.sequencer)
        await self.env.wait_for_completion()
        self.drop_objection()


@test()
class SimpleCpuWishboneUvmShadowFaultInjectionTest(SimpleCpuWishboneBaseTest):
    expected_runs = 0
    ...
    async def run_phase(self):
        ...
        await self.bfm.wishbone.load_shadow_program(base_program)
        await self.bfm.wishbone.start_program()

        await RisingEdge(cocotb.top.wb_clk_i)
        await self.bfm.wishbone.write(2, patched_program[2])
        observed_shadow = await self.bfm.wishbone.read(2)
        assert observed_shadow == patched_program[2], (
            f"wishbone_shadow_pyuvm_current_run: shadow[2] got {observed_shadow}, expected {patched_program[2]}"
        )

        await self.bfm.wishbone.run_until_halt(max_cycles=128)
        first_snapshot = await self.bfm.wishbone.sample_state()
        assert_snapshot_matches_model("wishbone_shadow_pyuvm_current_run", first_snapshot, model_initial)

        await self.bfm.wishbone.stop_program()
        await self.bfm.wishbone.start_program()
        await self.bfm.wishbone.run_until_halt(max_cycles=128)
        second_snapshot = await self.bfm.wishbone.sample_state()
        assert_snapshot_matches_model("wishbone_shadow_pyuvm_after_reload", second_snapshot, model_reloaded)
