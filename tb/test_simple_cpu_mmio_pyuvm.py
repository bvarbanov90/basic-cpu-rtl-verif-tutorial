...
class MmioProgramItem(uvm_sequence_item):
    def __init__(
        self,
        name="mmio_program_item",
        program=None,
...
class MmioLogicOpsSequence(uvm_sequence):
    async def body(self):
        item = MmioProgramItem(
            "mmio_logic_ops_item",
...
class MmioControlStatusSequence(uvm_sequence):
    async def body(self):
        program = [
            ins(OPC_LDI, 1),
            ins(OPC_STA, 0),
...
class MmioRandomProgramSequence(uvm_sequence):
    def __init__(self, name="mmio_random_program_sequence", seed=20260411):
        super().__init__(name)
        self.seed = int(seed)
...
class SimpleCpuMmioBfm(uvm_object):
    def __init__(self, name="simple_cpu_mmio_bfm", dut=None):
        super().__init__(name)
        self.dut = dut
        self._clock_started = False
        self.mmio = SimpleCpuMmioBus(dut)
    ...
    async def start_with_optional_state_observation(self, observe_control_states: bool, max_cycles: int) -> tuple[int, int]:
        if not observe_control_states:
            await self.mmio.start_program()
            return 0, 1

        await self.mmio.write(0x30, 0x01)
        seen_load = 0
        seen_run = 0
        for _ in range(max_cycles):
            control = await self.mmio.read(0x30)
            if (control & 0x2) != 0:
                seen_load = 1
            if (control & 0x1) != 0:
                seen_run = 1
    ...
    async def run_item(self, item: MmioProgramItem) -> MmioRunTrace:
        await self.reset()
        await self.mmio.load_shadow_program(item.program)

        if item.verify_shadow_readback:
            for addr, value in enumerate(item.program[:16]):
                observed = await self.mmio.read(addr)
        ...
        await self.mmio.run_until_halt(max_cycles=item.max_cycles)
        snapshot = await self.mmio.sample_state()
        return MmioRunTrace(
...
class SimpleCpuMmioDriver(uvm_driver):
    def build_phase(self):
        self.bfm = ConfigDB().get(self, "", "MMIO_CPU_BFM")
...
class SimpleCpuMmioScoreboard(uvm_component):
    def build_phase(self):
    ...
    @staticmethod
    def assert_equal(trace: MmioRunTrace, field: str, observed, expected):
    ...
    async def run_phase(self):
        while self.checked_runs < self.expected_runs:
            ...
            if trace.item_name == "mmio_control_status_item":
...
class SimpleCpuMmioAgent(uvm_component):
    def build_phase(self):
        self.sequencer = uvm_sequencer("sequencer", self)
        self.driver = SimpleCpuMmioDriver("driver", self)

    def connect_phase(self):
        self.driver.seq_item_port.connect(self.sequencer.seq_item_export)


class SimpleCpuMmioEnv(uvm_component):
    def build_phase(self):
        self.expected_runs = int(get_config(self, "EXPECTED_RUNS", 1))
        self.agent = SimpleCpuMmioAgent("agent", self)
        self.scoreboard = SimpleCpuMmioScoreboard("scoreboard", self)
    ...
    async def wait_for_completion(self, timeout_cycles=1024):
        ...
        raise AssertionError("MMIO pyuvm environment did not complete in time")


class SimpleCpuMmioBaseTest(uvm_test):
    expected_runs = 1

    def build_phase(self):
        bfm = SimpleCpuMmioBfm(dut=cocotb.top)
        ConfigDB().set(None, "*", "MMIO_CPU_BFM", bfm)
        ConfigDB().set(None, "*", "EXPECTED_RUNS", self.expected_runs)
        self.bfm = bfm
        self.env = SimpleCpuMmioEnv("env", self)
...
@test()
class SimpleCpuMmioUvmSmokeTest(SimpleCpuMmioBaseTest):
    async def run_phase(self):
        self.raise_objection()
        await MmioLogicOpsSequence("mmio_logic_ops_seq").start(self.env.agent.sequencer)
        await self.env.wait_for_completion()
        self.drop_objection()


@test()
class SimpleCpuMmioUvmRandomizedTest(SimpleCpuMmioBaseTest):
    async def run_phase(self):
        self.raise_objection()
        await MmioRandomProgramSequence("mmio_random_seq", seed=20260411).start(self.env.agent.sequencer)
        await self.env.wait_for_completion(timeout_cycles=4096)
        self.drop_objection()


@test()
class SimpleCpuMmioUvmControlStatusTest(SimpleCpuMmioBaseTest):
    async def run_phase(self):
        self.raise_objection()
        await MmioControlStatusSequence("mmio_control_status_seq").start(self.env.agent.sequencer)
        await self.env.wait_for_completion()
        self.drop_objection()


@test()
class SimpleCpuMmioUvmShadowFaultInjectionTest(SimpleCpuMmioBaseTest):
    expected_runs = 0
    ...
    async def run_phase(self):
        ...
        await self.bfm.mmio.load_shadow_program(base_program)
        await self.bfm.mmio.start_program()

        await RisingEdge(cocotb.top.clk)
        await self.bfm.mmio.write(2, patched_program[2])
        observed_shadow = await self.bfm.mmio.read(2)
        assert observed_shadow == patched_program[2], (
            f"mmio_shadow_pyuvm_current_run: shadow[2] got {observed_shadow}, expected {patched_program[2]}"
        )

        await self.bfm.mmio.run_until_halt(max_cycles=128)
        first_snapshot = await self.bfm.mmio.sample_state()
        assert_snapshot_matches_model("mmio_shadow_pyuvm_current_run", first_snapshot, model_initial)

        await self.bfm.mmio.stop_program()
        await self.bfm.mmio.start_program()
        await self.bfm.mmio.run_until_halt(max_cycles=128)
        second_snapshot = await self.bfm.mmio.sample_state()
        assert_snapshot_matches_model("mmio_shadow_pyuvm_after_reload", second_snapshot, model_reloaded)
