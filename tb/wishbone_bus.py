...
class SimpleCpuWishboneBus:
    ...
    async def wait_ready(self, *, max_cycles: int | None = None) -> None:
        ...
        for _ in range(wait_budget):
            ...
            if int(self.dut.wb_ack_o.value) == 1:
                return
            await RisingEdge(self.dut.wb_clk_i)
        raise AssertionError(f"wb_ack_o did not assert within {wait_budget} cycles")

    async def write(self, addr: int, value: int) -> None:
        self.dut.wb_cyc_i.value = 1
        self.dut.wb_stb_i.value = 0
        self.dut.wb_we_i.value = 1
        self.dut.wb_adr_i.value = addr & 0xFF
        self.dut.wb_dat_i.value = value & 0xFF
        await Timer(1, unit="ns")
        await RisingEdge(self.dut.wb_clk_i)
    ...
    async def read(self, addr: int) -> int:
        ...
        self.dut.wb_stb_i.value = 1
        await self.wait_ready()
        value = int(self.dut.wb_dat_o.value)
        await RisingEdge(self.dut.wb_clk_i)
        self.dut.wb_cyc_i.value = 0
        self.dut.wb_stb_i.value = 0
        self.dut.wb_adr_i.value = 0
    ...
    async def wait_for_control_state(self, *, run: int, load: int, max_cycles: int = 64) -> None:
        ...
        for _ in range(max_cycles):
            ...
            if (control & 0x3) == expected:
                return
            await RisingEdge(self.dut.wb_clk_i)
        raise AssertionError(f"Wishbone control register never reached load={load} run={run}")
    ...
    async def run_until_halt(self, max_cycles: int = 256) -> int:
        for cycle in range(max_cycles):
            ...
            if (status >> 4) & 0x1:
                return cycle
            await RisingEdge(self.dut.wb_clk_i)
        raise AssertionError("Wishbone DUT did not halt in time")
