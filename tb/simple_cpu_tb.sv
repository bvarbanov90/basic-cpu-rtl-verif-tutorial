...
module simple_cpu_tb;
    ...
    localparam [3:0] OPC_LDI = 4'h1;
    localparam [3:0] OPC_ADD = 4'h2;
    localparam [3:0] OPC_SUB = 4'h3;
    localparam [3:0] OPC_STA = 4'h4;
    localparam [3:0] OPC_LDA = 4'h5;
    localparam [3:0] OPC_JMP = 4'h6;
    localparam [3:0] OPC_JZ = 4'h7;
    localparam [3:0] OPC_HLT = 4'h8;
    localparam [3:0] OPC_AND = 4'h9;
    localparam [3:0] OPC_OR = 4'hA;
    localparam [3:0] OPC_XOR = 4'hB;
    localparam [3:0] OPC_SHL = 4'hC;
    localparam [3:0] OPC_SHR = 4'hD;
    localparam [3:0] OPC_CMP = 4'hE;
    ...
    task automatic cov_sample_step;
        ...
        begin
            ...
            if (carry_after) cov_carry_1 = cov_carry_1 + 1; else cov_carry_0 = cov_carry_0 + 1;
            if (neg_after) cov_neg_1 = cov_neg_1 + 1; else cov_neg_0 = cov_neg_0 + 1;
            if (overflow_after) cov_overflow_1 = cov_overflow_1 + 1; else cov_overflow_0 = cov_overflow_0 + 1;
    ...
    task automatic run_until_halt;
        ...
        begin
            ...
            while (!dbg_halted && cycle < max_cycles) begin
                ...
                pc_after = dbg_pc;
                cov_sample_step(opcode, operand, pc_before, pc_after, zero_before, dbg_zero, dbg_carry, dbg_neg, dbg_overflow);
    ...
    task automatic test_directed;
        begin
            ...
            dbg_mem_addr = 4'd0; #1; if (dbg_mem_data !== 8'd3) $fatal(1, "Directed: dmem[0] expected 3");
            dbg_mem_addr = 4'd1; #1; if (dbg_mem_data !== 8'd7) $fatal(1, "Directed: dmem[1] expected 7");
            dbg_mem_addr = 4'd2; #1; if (dbg_mem_data !== 8'd4) $fatal(1, "Directed: dmem[2] expected 4");
            dbg_mem_addr = 4'd3; #1; if (dbg_mem_data !== 8'd0) $fatal(1, "Directed: dmem[3] expected 0");
        ...
        begin
            ...
            for (opcode = 0; opcode <= MAX_VALID_OPCODE; opcode = opcode + 1) begin
                $fwrite(fd_json, "    \"%0d\": {\"zero0\": %0d, \"zero1\": %0d}",
                    opcode, cov_opcode_zero_cross[opcode][0], cov_opcode_zero_cross[opcode][1]);
                if (opcode < MAX_VALID_OPCODE) begin
                    $fwrite(fd_json, ",\n");
                end else begin
                    $fwrite(fd_json, "\n");
