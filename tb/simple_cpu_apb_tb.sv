...
module simple_cpu_apb_tb;
  ...
  task automatic apb_write;
    ...
    begin
      ...
      if (pready !== 1'b0) begin
        $fatal(1, "APB write setup observed PREADY asserted");
      end
      ...
      #1;
      if (pready !== 1'b1) begin
        $fatal(1, "APB write access did not hold PREADY through the active cycle");
      end
  ...
  task automatic apb_read_now;
    ...
    begin
      ...
      cov_setup_phases = cov_setup_phases + 1;
      if (pready !== 1'b0) begin
        $fatal(1, "APB read setup observed PREADY asserted");
      end
      ...
      if (pready !== 1'b1) begin
        $fatal(1, "APB read access observed PREADY deasserted");
      end
      data = prdata;
      if (addr[7:4] == 4'h0) begin
      ...
      #1;
      if (pready !== 1'b1) begin
        $fatal(1, "APB read access did not hold PREADY through the active cycle");
      end
  ...
  task automatic check_program_shadow;
    ...
    begin
      for (idx = 0; idx < 16; idx = idx + 1) begin
        apb_read_now(idx[7:0], observed);
        if (observed !== program_mem[idx]) begin
          $fatal(1, "APB shadow mismatch at imem[%0d]: got %0h expected %0h", idx, observed,
                 program_mem[idx]);
        end
      end
    end
  ...
  task automatic wait_for_halt;
    ...
    begin
      ...
      if (!status[4]) begin
        $fatal(1, "APB timeout: wrapper/core did not halt within %0d polls", max_polls);
      end
    end
  endtask
  ...
  task automatic wait_for_run_state;
    ...
    begin
      ...
      if (control != 8'h01) begin
        $fatal(1, "APB timeout: wrapper did not reach RUN within %0d polls", max_polls);
      end
    end
  endtask
  ...
  task automatic run_reference_model;
    ...
    begin
      ...
      if (!model_halted) begin
        $fatal(1, "APB reference model timeout");
      end
  ...
  task automatic check_against_reference;
    ...
    begin
      apb_read_now(ADDR_STATUS, status);
      apb_read_now(ADDR_ACC, observed);
      if (observed !== ref_acc) begin
        $fatal(1, "APB ACC mismatch: got %0h expected %0h", observed, ref_acc);
      end

      apb_read_now(ADDR_PC, observed);
      if (observed[3:0] !== ref_pc) begin
        $fatal(1, "APB PC mismatch: got %0h expected %0h", observed[3:0], ref_pc);
      end

      if (status[0] !== ref_zero) begin
        $fatal(1, "APB ZERO mismatch");
      end
      if (status[1] !== ref_carry) begin
        $fatal(1, "APB CARRY mismatch");
      end
      if (status[2] !== ref_neg) begin
        $fatal(1, "APB NEG mismatch");
      end
      if (status[3] !== ref_overflow) begin
        $fatal(1, "APB OVERFLOW mismatch");
      end
      if (status[4] !== ref_halted) begin
        $fatal(1, "APB HALTED mismatch");
      end

      for (idx = 0; idx < 16; idx = idx + 1) begin
        apb_read_now(8'h20 + idx[7:0], observed);
        if (observed !== ref_dmem[idx]) begin
          $fatal(1, "APB dmem[%0d] mismatch: got %0h expected %0h", idx, observed, ref_dmem[idx]);
        end
      end
    end
  endtask

  task automatic run_program_via_apb;
    input integer max_cycles;
    reg [7:0] control;
    begin
      ...
      if (control !== 8'h00) begin
        $fatal(1, "APB control expected HOLD state before programming, got %0h", control);
      end
      ...
      if (control[1] !== 1'b1) begin
        $fatal(1, "APB expected loader-active state immediately after start, got %0h", control);
      end

      wait_for_halt(max_cycles + 64);
      apb_read_now(ADDR_CONTROL, control);
      if (control[0] !== 1'b1) begin
        $fatal(1, "APB expected RUN state after execution, got %0h", control);
      end
  ...
  task automatic write_apb_coverage_reports;
    input coverage_pass;
    integer fd_json;
    integer fd_csv;
    begin
      fd_json = $fopen("sim_build/apb_coverage.json", "w");
      if (fd_json == 0) begin
        $fatal(1, "Could not open sim_build/apb_coverage.json for write");
      end
      ...
      fd_csv = $fopen("sim_build/apb_coverage.csv", "w");
      if (fd_csv == 0) begin
        $fatal(1, "Could not open sim_build/apb_coverage.csv for write");
      end
  ...
  task automatic report_and_check_apb_coverage;
    reg coverage_pass;
    begin
      coverage_pass = 1'b1;
      ...
      if (cov_program_runs < 4) begin
        coverage_pass = 1'b0;
        $display("[APB-COVERAGE][MISS] program_runs >= 4");
      end
      if (cov_shadow_writes < (16 * cov_program_runs)) begin
        coverage_pass = 1'b0;
        $display("[APB-COVERAGE][MISS] shadow_writes per run");
      end
      if (cov_shadow_reads < (16 * cov_program_runs)) begin
        coverage_pass = 1'b0;
        $display("[APB-COVERAGE][MISS] shadow_reads per run");
      end
      if (cov_dmem_reads < (16 * cov_program_runs)) begin
        coverage_pass = 1'b0;
        $display("[APB-COVERAGE][MISS] dmem_reads per run");
      end
      if (cov_status_reads < cov_program_runs) begin
        coverage_pass = 1'b0;
        $display("[APB-COVERAGE][MISS] status_reads per run");
      end
      if (cov_acc_reads < cov_program_runs) begin
        coverage_pass = 1'b0;
        $display("[APB-COVERAGE][MISS] acc_reads per run");
      end
      if (cov_pc_reads < cov_program_runs) begin
        coverage_pass = 1'b0;
        $display("[APB-COVERAGE][MISS] pc_reads per run");
      end
      if (cov_control_start_writes < cov_program_runs) begin
        coverage_pass = 1'b0;
        $display("[APB-COVERAGE][MISS] control_start_writes per run");
      end
      if (cov_control_stop_writes < cov_program_runs) begin
        coverage_pass = 1'b0;
        $display("[APB-COVERAGE][MISS] control_stop_writes per run");
      end
      if (cov_setup_phases != cov_access_phases) begin
        coverage_pass = 1'b0;
        $display("[APB-COVERAGE][MISS] setup_phases must equal access_phases");
      end
      if (cov_access_phases != (cov_read_accesses + cov_write_accesses)) begin
        coverage_pass = 1'b0;
        $display("[APB-COVERAGE][MISS] access_phases must equal total transactions");
      end
      if (!cov_hold_state_seen || !cov_load_state_seen || !cov_run_state_seen) begin
        coverage_pass = 1'b0;
        $display("[APB-COVERAGE][MISS] hold/load/run states all seen");
      end

      write_apb_coverage_reports(coverage_pass);
      $display("[APB-COVERAGE] wrote sim_build/apb_coverage.json and sim_build/apb_coverage.csv");

      if (!coverage_pass) begin
        $fatal(1, "APB coverage goals not met.");
      end
    end
  endtask

  task automatic test_apb_smoke;
    begin
      clear_program();
      ...
      run_program_via_apb(80);
      $display("[PASS] apb_smoke_and_shadow_readback");
    end
  endtask

  task automatic test_apb_reprogram_sequence;
    begin
      clear_program();
      ...
      run_program_via_apb(96);
      $display("[PASS] apb_reprogram_and_shift_status");
    end
  endtask

  task automatic test_apb_illegal_opcode;
    begin
      clear_program();
      program_mem[0] = 8'hF0;
      program_mem[1] = ins(OPC_LDI, 4'd9);
      run_program_via_apb(16);
      if (ref_pc !== 4'd0) begin
        $fatal(1, "APB illegal-opcode reference expected PC 0");
      end
      $display("[PASS] apb_illegal_opcode_halts");
    end
  endtask

  task automatic test_apb_jump_sub_cmp_sequence;
    begin
      clear_program();
      ...
      run_program_via_apb(64);
      $display("[PASS] apb_jump_sub_cmp_reference_compare");
    end
  endtask

  task automatic test_apb_shadow_fault_injection;
    reg [7:0] observed;
    begin
      clear_program();
      ...
      if (observed !== ins(OPC_LDI, 4'd9)) begin
        $fatal(1, "APB fault-injection shadow update did not stick");
      end
      ...
      if (ref_dmem[3] !== 8'h04) begin
        $fatal(1, "APB fault injection expected first run to preserve original shadow image");
      end

      apb_write(ADDR_CONTROL, 8'h00);
      apb_read_now(ADDR_CONTROL, observed);
      if (observed !== 8'h00) begin
        $fatal(1, "APB fault injection expected HOLD after explicit stop");
      end
      ...
      if (ref_dmem[3] !== 8'h09) begin
        $fatal(1, "APB fault injection expected second run to observe the updated shadow image");
      end

      $display("[PASS] apb_shadow_fault_injection_isolated_until_reload");
    end
  endtask
  ...
  task automatic test_external_program;
    ...
    begin
      ...
      run_program_via_apb(256);
      cov_external_program_runs = cov_external_program_runs + 1;
      $display("[PASS] apb_external_program_reference_compare (%0s)", hex_path);
    end
  endtask

  initial begin
`ifndef NO_WAVES
    $dumpfile("sim_build/simple_cpu_apb_tb.vcd");
    $dumpvars(0, simple_cpu_apb_tb);
`endif

    cov_init();
    global_reset();
    test_apb_smoke();
    test_apb_reprogram_sequence();
    test_apb_illegal_opcode();
    test_apb_jump_sub_cmp_sequence();
    test_apb_shadow_fault_injection();

    if ($value$plusargs("PROGRAM_HEX=%s", external_program_hex)) begin
      $display("[INFO] Running APB external program from %0s", external_program_hex);
      test_external_program(external_program_hex);
    end

    report_and_check_apb_coverage();
    $display("[PASS] all apb tests");
    $finish;
  end
endmodule
